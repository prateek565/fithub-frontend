...
export const login = (creds) => async (dispatch) => {
  ...
  try {
    let res = await axios.post("https://fitness-hub-backend.onrender.com/user/login", creds);
...
export const ForegetPassword = (email) => async (dispatch) => {
  ...
  try {
    let res = await axios.post("https://fitness-hub-backend.onrender.com/user/reset-password/getOtp", {email: email});
...
export const getUserData = (email) => async (dispatch) => {
 // console.log(email)
  ...
  try {
    let res1 = await axios.get("https://fitness-hub-backend.onrender.com/cart/" + email );
    let res2 = await axios.get("https://fitness-hub-backend.onrender.com/user/" + email);

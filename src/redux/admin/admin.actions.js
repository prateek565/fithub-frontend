...
export const ACTION_GET_ADMIN = () => async (dispatch) => {
  ...
  // console.log("data")
  try {
    let res1 = await axios.get("https://fitness-hub-backend.onrender.com/cart");
    let res2 = await axios.get("https://fitness-hub-backend.onrender.com/user");
...
console.log(payload)
    try {
      await axios.delete(`https://fitness-hub-backend.onrender.com/products/${payload}`, {
        headers: {
...
export const ACTION_ADD_PRODUCT =
  ...
  async (dispatch) => {
    ...
    try {
      await axios.post(`https://fitness-hub-backend.onrender.com/products`, {
        headers: {
          "Content-Type": "application/json",
        },
        data: {
      ...
      //  await axios.post(`https://fitness-hub-backend.onrender.com/${payload.value}`, payload.area);
